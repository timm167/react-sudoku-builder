    ...
    return (
        ...
        <div className={`${state.isRequestingSum ? '' : 'hidden'} submit-holder`}>
            <form onSubmit={handleSubmit} className='submit-holder'>
                <input type="number" name="sumInput" id="sumInput" className='form-input'/>
        ...
        <div className='killer-buttons'>
            <div className={`inner-killer-buttons ${state.killerMode ? '' : 'hidden'} `}>
                <button 
                    className={`killer-button delete-box-button ${state.deletingBox ? 'highlight-button' : ''}`}
                    onClick={handleDeleteBoxClick}
                >
                    {`${state.deletingBox ? 'Select a Box' : 'Delete Box'}`}
                </button>
                <button 
                    className={`killer-button set-box-total-button ${state.settingBoxTotal ? 'highlight-button' : ''}`}
                    onClick={handleSetBoxSumClick}
                >
                    {`${state.settingBoxTotal ?  'Select a Box' : 'Set Box Sum'}`}
                </button>
                <button ref={buttonInputRefs.newBoxButton} className="killer-button create-box-button" onClick={handleCreateBoxClick}>
